...
ENV VIRTUAL_ENV /opt/fastsocket-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/fastsocket/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/fastsocket/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/fastsocket/ && touch src/fastsocket/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/fastsocket/
...
# Expose the app.
ENTRYPOINT ["/opt/fastsocket-env/bin/fastsocket"]
CMD []
